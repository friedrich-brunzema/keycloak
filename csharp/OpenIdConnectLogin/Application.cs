...
namespace gt
{
    internal class Application
    {
        ...
        // https://login.qa.oneomics.net/auth/realms/oneomics/.well-known/openid-configuration
        // has a list of discovery information.  Replace realm name and server name if needed.

        private const string KeyCloakUrlStaging = "https://login.dev.oneomics.net/auth/";

        private const string KeyCloakUrlVerification =
            "https://login.qa.oneomics.net/auth/";

        private const string KeyCloakUrlBeta = "https://login.beta.sciex.com/auth/";

        private const string KeyCloakVerificationBaseAddress =
            "https://login.qa.oneomics.net";

        private const string KeyCloakBetaBaseAddress = "https://login.beta.sciex.com";

        private const string KeyCloakUrlTokenEndPointVerificationQa =
            "https://login.qa.oneomics.net/auth/realms/oneomics/protocol/openid-connect/token";

        private const string KeyCloakUrlTokenEndPointVerificationBeta =
            "https://login.beta.sciex.com/auth/realms/oneomics/protocol/openid-connect/token";
        ...
        private const string ClientSecretVerification =
            "ff36904c-75b7-4086-9eda-607fe3e2e92e";
        ...
        {
            Console.WriteLine(
                "OpenIdConnect JWT-Token getter hack, v1.0  (c) SCIEX 2019" +
                Environment.NewLine);
            ...
            try
            {
                ...
                formattedTokensAsJson =
                    JToken.Parse(tokensJson).ToString(Formatting.Indented);
                _refreshToken = JObject.Parse(formattedTokensAsJson)["refresh_token"]
                    .ToString();
                accessToken = JObject.Parse(formattedTokensAsJson)["access_token"]
                    .ToString();
            }
            catch
            {
        ...
        private string InitialAuthRequest()
        {
            ...
            request.Parameters.Add(new Parameter("User-Agent", UserAgent,
        ...
        private async Task PostLoginRequest(string html, string username, string password)
        {
            ...
            cookieContainer.Add(new Uri(KeyCloakBetaBaseAddress),
                new Cookie("AUTH_SESSION_ID", _authSessionIdCookieValue));
            cookieContainer.Add(new Uri(KeyCloakBetaBaseAddress),
                new Cookie("KC_RESTART", _kcRestartCookieValue));
        ...
        {
            var request =
                new RestRequest("https://login.qa.oneomics.net/auth", Method.POST);
            var httpClientHandler = new HttpClientHandler
            {
                AllowAutoRedirect = false,
                AutomaticDecompression =
            ...
            var rq = new HttpRequestMessage(HttpMethod.Post,
                new Uri("realms/oneomics/protocol/openid-connect/logout"));
            rq.Content = new FormUrlEncodedContent(nvc);
            rq.Headers.Add("User-Agent", UserAgent);
            rq.Headers.Add("Connection", "keep-alive");
            rq.Headers.Add("Cache-control", "no-cache");
            ...
            List<Parameter> parameters)
        {
            ...
            doc.LoadHtml(html);
            var nd = doc.DocumentNode.SelectSingleNode(
